...
const CarnetPublicoPage = () => {
    ...
    const load = async () => {
      ...
      try {
        const { data, error: qErr } = await supabase
          ...
          .select("carnet_pdf_url, carnet_poliza_url")
          .eq("id", polizaId)
          .maybeSingle();
        if (qErr) throw qErr;
        // Preferimos el PDF; carnet_poliza_url queda como respaldo (ahora también apunta al PDF).
        let pdf = (data as any)?.carnet_pdf_url || (data as any)?.carnet_poliza_url || null;
        ...
        if (!pdf) throw new Error("No se pudo obtener el carnet");
        setUrl(pdf);
      } catch (e: any) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.download = `carnet-${polizaId}.pdf`;
